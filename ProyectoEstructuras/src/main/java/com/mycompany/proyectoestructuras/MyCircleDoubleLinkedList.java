...
public class MyCircleDoubleLinkedList<T> {
    ...
    public void add(T dato) {
        ...
        if (head == null) {                 // Si la lista está vacía, el nuevo nodo es cabeza y se enlaza a sí mismo
            ...
            CircularDoubleNode<T> cola = head.previous; // Obtener la cola desde la referencia de cabeza
            cola.next = nuevoNodo;       // El último nodo apunta al nuevo nodo
            nuevoNodo.previous = cola;        // El nuevo nodo apunta al anterior (antigua cola)
            nuevoNodo.next = head;     // El nuevo nodo apunta a la cabeza
            head.previous = nuevoNodo;      // La cabeza apunta de nuevo al nuevo nodo (nueva cola)
        }
    ...
    public T getLast() {
        ...
        return head.previous.data; // La cola está almacenada en cabeza.anterior
    }
    ...
    public T removeFirst() {
        ...
        T datoEliminado = head.data;
        if (head.next == head) { // Si solo hay un elemento
            head = null;
        } else {
            CircularDoubleNode<T> cola = head.previous;
            head = head.next;         // La cabeza ahora apunta al segundo nodo
            head.previous = cola;            // La nueva cabeza apunta a la cola
            cola.next = head;           // La cola apunta de nuevo a la nueva cabeza
        }
        size--;
    ...
    public T removeLast() {
        ...
        T datoEliminado = head.previous.data;
        if (head.next == head) { // Si solo hay un elemento
            head = null;
        } else {
            CircularDoubleNode<T> cola = head.previous;
    ...
    public void printListReverse() {
        ...
        CircularDoubleNode<T> actual = head.previous; // Comienza desde la cola
        do {
            System.out.print(actual.data + " <-> ");
